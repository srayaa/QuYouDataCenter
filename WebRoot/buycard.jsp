...
<body>
		...
		<div class="page-content" id="main-stack">
			<div class="w-nav navbar" data-collapse="all"
				...
				<div class="w-container">
					...
					<div class="navbar-title">购卡记录</div>
	...
	/********************************************************************/
		...
		// b = a - 86400 * 3;
			doGetWithValidate('/buyCardInMobile.do?type=1&start_index=' + start + '&end_index=' + pageSize, null, function(d) {
				if (d.data && d.data.length) {
					for (var i = 0; i < d.data.length; i++) {
						$('.list-messages').append('<li class="list-message" data-ix="list-item" style="opacity: 1; transform: translateX(0px) translateY(0px); transition: opacity 500ms cubic-bezier(0.23, 1, 0.32, 1), transform 500ms cubic-bezier(0.23, 1, 0.32, 1);">' + '<a class="w-clearfix w-inline-block" href="#"><div class="column-right" style="padding-left: 20px;">' + '  <div class="message-title">' + d.data[i]['sellerName'] + '</div>' + '  <div class="message-text">' +  getLocalTime( new Date(d.data[i]['sellTimeInMb'])) + '  用户ID: ' + d.data[i]['sellerId'] + '  售卡: ' + (parseInt(d.data[i]['cardLCount']) + parseInt(d.data[i]['cardFCount'])) + '张' + '</div>' + '</div></a>' + '</li>');
					}
				} else {
